...
@section('content')
    <div class="content-header" style="display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;">
        <div>
            <h1 style="font-size:24px;margin-bottom:5px;">Offer Types</h1>
            <nav style="font-size:14px;color:var(--text-muted);">
                <a href="{{ route('admin.dashboard') }}" style="color:var(--primary);text-decoration:none;">Home</a> / Offer Types
            </nav>
        </div>
        <a href="{{ route('admin.type-offers.create') }}" class="btn btn-primary"
            ...
            <table class="table" style="width:100%;border-collapse:collapse;color:var(--text-main);">
                <thead>
                    <tr style="text-align:left;border-bottom:1px solid var(--border-color);">
                        ...
                        <th style="padding:15px;color:var(--text-muted);font-weight:500;font-size:13px;">OFFERS</th>
                ...
                <tbody>
                    @forelse($types as $i => $type)
                        <tr style="border-bottom:1px solid var(--border-color);">
                            ...
                            <td style="padding:15px;">{{ $type['title_en'] ?? '-' }}</td>
                            <td style="padding:15px;">{{ $type['title_ar'] ?? '-' }}</td>
                            <td style="padding:15px;">{{ $type['offers_count'] ?? 0 }}</td>
                            <td style="padding:15px;">
                                <div style="display:flex;gap:8px;">
                                    <a href="{{ route('admin.type-offers.edit', $type['id']) }}" class="btn-icon"
                                        style="background:rgba(245,158,11,0.1);color:#f59e0b;padding:8px;border-radius:6px;text-decoration:none;"><i
                                            class="fas fa-edit"></i></a>
                                    <form action="{{ route('admin.type-offers.destroy', $type['id']) }}" method="POST"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" style="padding:30px;text-align:center;color:var(--text-muted);">No types found</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
